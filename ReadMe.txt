~~~~Fight Night Duo~~~~~

1)Alpha requirements:
	tech: not sure hasnt failed on any windows device yet not tested on mac/linux
	software: love2d game engine (download:" http://love2d.org ")
	etc: Patience and understanding
2)Installation:
	love2d: download from link above and follow instructions provided on web
	Fight Night Duo:  no installation required (unzip file)
3)Running The Alpha
	a)once unzipped you will see the files "readme.txt"(this) and "check.love"(the game).
	b)after installing love2d the .love extension should be associated with "love.exe"
	c)Now to actually run the game :D if the file type is associated already then simply double click the
		.love file, otherwise make a shortcut to love2d on the desktop, or somewhere else you can find it
		and drag the .love file onto the shortcut. This should launch the game.
4)Controls
	found in game under the help page
5)Features
	a) 2 player combat
	b)knockback + respawn
	c)hud displaying player stats(WIP)
	d)multiple maps (WIP)
	e)multiple classes (NYI)
6)Credits
	programming:Craigory Coppola
	various assistance: Jimmy Hardy and Vincent Etue
	art: Craigory Coppola
	sound: Craigory Coppola
	concepts: Craigory Coppola w/ assistance from Vincent Etue
	bug testing: Craigory Coppola, Jimmy Hardy, and Vincent Etue
